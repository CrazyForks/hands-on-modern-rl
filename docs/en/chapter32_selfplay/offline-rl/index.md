...
DPO from Chapter 15 also uses a fixed dataset without generating new answers online, so it can be broadly viewed as a form of offline policy optimization[^dpo]. However, it still has important differences from traditional Offline RL.
