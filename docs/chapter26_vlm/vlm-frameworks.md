...
VisPlay 是一个很有创意的 VLM RL 框架，它的核心思想是让两个模型通过 RL **互相博弈、协同进化**——一个负责"出题"（Questioner），一个负责"答题"（Reasoner）。这和第 29 章讨论的 Self-Play 思想一脉相承，但专门为视觉场景设计。
...
VISTA-Gym 可以和 GRPO 自然结合。对同一个图片+问题，模型生成多组推理链（每组包含不同的工具调用序列），然后用规则奖励评估每组的质量，计算组内相对优势，更新策略。这和第 16 章的 GRPO 完全一致——只是生成的"回答"从纯文本变成了"工具调用序列 + 最终回答"。
...
| | VLM GRPO（24.3 动手实验） | EasyR1（24.4 GeoQA 实验） | VisPlay / VISTA-Gym | VeRL-Omni |
| --- | --- | --- | --- | --- |
...
VLM-RL 在机器人领域的应用前景最为广阔。机器人需要从摄像头输入中理解环境，然后做出操作决策。和第 26.1 节具身智能讨论的连续控制不同，VLM-RL 的核心是"用视觉理解来指导动作"——不是直接从像素到力矩，而是先理解"面前是什么物体"，再决定"该怎么操作"。
...
**仿真精调**阶段，针对目标机器人的具体场景做精细化训练。这一步会引入域随机化（Domain Randomization，回顾第 26.1 节具身智能）——随机化光照、纹理、物体位置等参数，让策略在各种条件下都能工作。
...
| 第 16 章：GRPO        | Token 序列      | 离散（token）    | 规则验证                | GRPO      |
| 第 20 章：Agentic RL | 文本 + 工具轨迹 | 工具调用 / token | 结果奖励 + 过程奖励     | PPO/GRPO  |
| 第 24 章：VLM RL     | 图像 + Token    | 离散（token）    | 规则 + 模型 + grounding | GRPO      |

贯穿始终的核心思想没有变——策略梯度定理（第 6 章）、Actor-Critic 架构（第 7 章）、PPO 的裁剪稳定性（第 8 章）、GRPO 的组内优势（第 16 章）。变化的是输入的模态、动作的空间和奖励的来源。这就是为什么我们在前几章花了大量篇幅打理论基础——这些基础在多模态场景中完全适用。
...
把本章的 VLM RL 和[第 20 章](../chapter22_agentic/overview)的 Agent RL 结合，会面临三个额外的挑战：
...
**3. 跨模态 Credit Assignment。** 在一条 10 轮的轨迹中，第 2 轮的视觉理解错误可能导致第 5 轮的工具调用失败。这比纯文本 Agent 的 credit assignment 更难，因为跨模态的错误传播链更长、更隐晦。[第 20 章](../chapter22_agentic/multi-turn-rl)讨论的 ORM vs PRM 取舍在这里更加突出。
...
2. **再练工具调用**：用[第 20 章的工具调用 RL](../chapter22_agentic/tool-use-and-trajectory)训练基本的工具使用模式
