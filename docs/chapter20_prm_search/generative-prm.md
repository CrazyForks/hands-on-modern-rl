...
判别式 PRM 需要为每个步骤准备分类标签，迁移到新领域时往往还要补充标注。2025 年 4 月，[ThinkPRM](https://arxiv.org/abs/2504.16828) 改用 LLM 生成自然语言评价：模型先解释当前步骤是否成立，再给出判断。这类方法称为**生成式 PRM**（Generative PRM）。

ThinkPRM 的实验重点在于标签效率：使用显著少于 PRM800K 的人工标签，在 MATH 等任务上训练可以解释判断依据的 verifier。本节先介绍输出形式和训练方法，再看标签效率与 verifier compute，最后比较它与判别式 PRM、结果奖励的关系。
...
这说明还需要比较两个问题：过程奖励是否是训练推理行为的必要条件，以及结果奖励在多大规模下能够间接强化自我检查。
...
[17.4 形式化 PRM Verifier](./formal-prm) 把可形式化的步骤交给 Lean4 等外部证明检查器，以确定性检查替代模型判断。
