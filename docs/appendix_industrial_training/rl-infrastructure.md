...
TRL（Transformer Reinforcement Learning）是 HuggingFace 生态内的 RL 训练库 [^trl]。前面各章的 DPO（第 15 章）和 GRPO（第 16 章）实验都用 TRL 完成。它的定位和前面三个框架不同：TRL 不是分布式编排系统——它不做 Ray 调度，不做 rollout engine 与 trainer 的进程分离，也不做跨 GPU 的 weight sync。它把 DPO/PPO/GRPO/REINFORCE++ 的训练循环封装成 `DPOTrainer`、`GRPOTrainer` 等 Trainer 类，在单机或少量 GPU 上运行 [^trl]。
