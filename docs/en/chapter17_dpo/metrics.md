...
`beta` is the regularization strength controlling how far the policy is allowed to drift from the reference model. In [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py), it is set by `DPOConfig(beta=0.1)`.
