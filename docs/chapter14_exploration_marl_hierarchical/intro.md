...
其中 $r_t^{\text{ext}}$ 是环境在第 $t$ 步给出的奖励，$r_t^{\text{int}}$ 是智能体根据新颖度计算的奖励，$\beta$ 决定探索信号占多大比重。例如外部奖励为 0、内在奖励为 0.4、$\beta=0.1$ 时，总奖励就是 0.04。关键在于内在奖励需要满足两点：
...
得到特征 $\Phi(s_t)$ 后，前向模型 $f_\psi$ 根据当前特征和动作预测下一状态特征：
...
刚进入陌生区域时，模型预测不准，这个距离较大；同类状态反复出现后，前向模型逐渐学会预测，奖励随之降低。系数 $1/2$ 只用于简化求导，不改变奖励的排序。

训练时把策略、逆向模型和前向模型的损失加在一起：

$$\mathcal{L} = \mathcal{L}_{\text{policy}}(\theta) + \lambda_{\text{inv}}\,\mathcal{L}_{\text{inv}}(\phi) + \lambda_{\text{fwd}}\,\mathcal{L}_{\text{fwd}}(\psi)$$

$\lambda_{\text{inv}}$ 和 $\lambda_{\text{fwd}}$ 控制两个辅助任务的权重。逆向模型帮助特征保留与动作有关的信息，前向模型提供新颖度，策略则使用加入内在奖励后的总奖励更新。
...
Random Network Distillation（Burda et al. 2018）不预测下一帧。它先固定一个随机初始化、永不更新的目标网络 $\hat f(s)$，再训练预测网络 $f_\psi(s)$ 去拟合同一个输出：

$$\mathcal{L}_{\text{RND}}(\psi) = \mathbb{E}_s\bigl[\|f_\psi(s) - \hat{f}(s)\|^2\bigr]$$

$$r^{\text{int}}(s) = \|f_\psi(s) - \hat{f}(s)\|^2$$

第一行是预测网络的训练损失，第二行把同一个误差用作探索奖励。已访问过的状态参与过多次训练，误差通常较小；新状态没有被拟合，误差通常较大。随机目标网络不表示任务语义，它只为每个输入提供固定目标。
...
这是便于理解的简化写法。$k$ 表示参与比较的近邻数，$N(s_i)$ 表示相似状态的访问次数，$c$ 控制重复访问带来的衰减。邻居越近、访问越频繁，单回合新颖度就越低。
