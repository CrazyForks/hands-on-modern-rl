...
不只对最终答案做对齐，还对 CoT 中的每个步骤做对齐。这就是 [第 16 章 PRM](../chapter18_grpo/grpo-family) 的核心思想——用过程奖励模型评估每个推理步骤。
...
工业实践的趋势是：**Visible CoT 占据主流，但对齐研究开始关注推理过程的内部对齐**——不只是 CoT 文本对齐，更是模型内部状态的对齐。这与 [第 13 章奖励黑客与对齐失败](../chapter15_rlhf/evaluation) 的内容紧密相关。
