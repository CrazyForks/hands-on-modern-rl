...
**定向合成。** 针对占比最大的错误类型，用 [20.4 节的轨迹合成方法](./tool-use-and-trajectory) 生成一批"正确定位"的训练数据。合成数据中强化"理解代码结构 → 定位相关文件"的能力。
...
改进方向：不是继续在工具调用上花力气，而是加强 Agent 的规划能力（参考 [20.3 轨迹信用分配](./credit-assignment)）和代码理解能力。评测的归因分析会直接指向这些方向。
