# 17.2 判别式 PRM 如何评价推理步骤

17.1 节把过程奖励解释为对中间步骤的反馈。判别式 PRM 将这项工作建模为分类任务：输入题目和当前推理步骤，输出该步骤正确、错误或中性的概率。

本节以 OpenAI 的 [Let's Verify Step by Step](https://arxiv.org/abs/2305.20050)（Lightman et al. 2023）为主线，依次介绍过程监督数据、分类器训练、回答选择与 RL 奖励，以及标注和步骤切分带来的成本。
...
判别式 PRM 的主要成本来自逐步标注、跨领域迁移和步骤切分。[17.3 生成式 PRM 如何解释推理步骤](./generative-prm) 改用自然语言生成评价，尝试用较少的人工标签覆盖新的任务。
