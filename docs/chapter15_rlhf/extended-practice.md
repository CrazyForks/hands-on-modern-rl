...
这一章到这里就完整闭环了：base model 不是 assistant，SFT 给它行为起点，RM 给它偏好方向，PPO 让它按奖励练习，评估和数据飞轮负责防止它学歪。下一章进入[第 14 章 DPO](../chapter17_dpo/intro)，先学习如何直接使用偏好数据更新模型；工业规模的数据、系统与稳定性问题将在第 18 章统一梳理。
