...
**粒度越细，模型越能区分哪些决策真正重要，学习效率越高。** 后面 GRPO（第 15 章）和 Agentic RL（第 19 章）会进一步利用这一特性，在更长、更复杂的轨迹上做细粒度信用分配。
...
这和第 8 章 PPO 的直觉完全一致。区别只是：动作从“LunarLander 的推力方向”变成了“词表里的某个 token”。
