---
title: 'Supplement: DPO Derivation'
---

# Supplement: DPO Derivation
...
Returning to [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py) from the previous section, the `preference_data.json` loaded by the code is in exactly this format. The data loading part parses the JSON file into `prompt`, `chosen`, and `rejected` fields, corresponding to $x$, $y_w$, and $y_l$ in the notation:
...
Returning to the code in [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py), `DPOTrainer` receives only a `model` parameter at initialization:
