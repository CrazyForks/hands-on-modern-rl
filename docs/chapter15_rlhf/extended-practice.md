...
这一章到这里就完整闭环了：base model 不是 assistant，SFT 给它行为起点，RM 给它偏好方向，PPO 让它按奖励练习，评估和数据飞轮负责防止它学歪。下一章进入[第 14 章现代 LLM RL 工业实践](../chapter16_llm_rl_industrial/intro)，从这条经典 RLHF 流水线出发，讨论奖励、优化器和分布式训练如何走向工业规模。
