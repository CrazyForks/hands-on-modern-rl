...
16.4 节使用模式开关和思考预算控制推理计算，但这些参数仍然需要用户或系统预先设置。自适应思考把选择权交给模型：简单任务直接回答，复杂任务增加推理深度，并允许外部预算限制最长思考时间。

本节先说明这种连续控制与 Hybrid Thinking 的区别，再用研究任务观察模型怎样分配计算，随后讨论安全与成本，最后连接过程对齐和 Agent 系统。
...
[16.6 推理链如何展示与对齐](./cot-visibility-alignment) 接着讨论一个直接问题：模型生成了内部推理以后，哪些内容应当展示，哪些内容需要监控或隐藏。
