...
第 3 章走了路线一——先学 $Q(s,a)$ 给每个动作打分，再选分数最高的（回顾：[Q(s,a) 与贪心策略](../chapter03_mdp/value-q)）。经典 DQN 在 CartPole 和 Atari 上效果很好，但它需要枚举网络输出的所有动作再取最大值，因此直接适用于有限离散动作空间。
...
这正是第 2 章[策略目标 $J(\theta)$](../chapter03_mdp/policy-value)的核心思路——定义一个策略目标函数 $J(\theta)$，然后直接优化参数 $\theta$ 使 $J(\theta)$ 最大。本章将深入这条路线，从策略梯度定理到 REINFORCE 算法，再到降低方差的基线方法。

::: tip 前置知识回顾
本章会频繁用到以下概念，如果记不清了，点进去复习一下再继续：

- [策略 $\pi_\theta$ 与目标函数 $J(\theta)$](../chapter03_mdp/policy-value)——参数化策略怎么表示？怎么定义"策略有多好"？
...
在拆数学之前，值得先退一步，看清本章的方法和第 5 章的 DQN 有什么本质不同。这不是两条平行的路——它们在解决问题的方式、能处理的问题类型、以及工程特性上都有根本差异。

### 核心思路

**Value-Based**（第 5 章 DQN）的核心思路是**打分**：学一个 $Q(s,a)$，给每个动作打分，然后选分数最高的。策略是隐式的——不直接学"该做什么"，而是通过 $\arg\max_a Q(s,a)$ 间接得到（回顾：[Q(s,a) 与贪心策略](../chapter03_mdp/value-q)）。

**Policy-Based**（本章）的核心思路是**直接学策略**：参数化 $\pi_\theta(a|s)$，直接输出每个动作的概率，然后优化参数 $\theta$ 使期望回报最大（回顾：[策略目标 $J(\theta)$](../chapter08_policy_gradient/policy-gradient)）。策略是显式的——不需要中间的打分步骤。
...
经典 DQN 一次输出所有候选动作的 $Q$ 值，再通过 $\arg\max$ 选择动作。CartPole 只有 2 个动作，这种枚举很直接；机械臂的关节力矩却来自连续空间 $[-10, 10]^6$，无法让网络为无限多个动作逐一输出分数。连续动作的价值方法需要额外求解 $\arg\max_a Q(s,a)$，工程上通常引入显式 Actor 来近似这个优化过程。

Policy-Based 方法没有这个限制。它不比较动作的分数，而是直接输出概率分布——离散动作用 Softmax，连续动作用高斯分布。同一个策略梯度公式，换一个输出层就能从"左/右"切换到"连续力矩"。

### 探索机制

DQN 的策略是确定性的（永远选 $\arg\max$），探索必须靠外部手段——第 5 章用的是 $\varepsilon$-greedy：以概率 $\varepsilon$ 随机选动作，否则选 $Q$ 值最大的。$\varepsilon$ 需要手动调度，太大浪费，太小不够探索（回顾：[DQN 的三个组件](../chapter07_dqn/dqn-components)）。
...
本章介绍的 REINFORCE 是 **on-policy** 的：梯度估计中的 $\mathbb{E}_{\pi_\theta}$ 需要用当前策略 $\pi_\theta$ 产生的数据来估计。策略更新后，旧数据不能像 DQN 的经验回放那样直接反复使用，因此基础 REINFORCE 的数据效率通常低于 DQN。后续的离策略策略梯度方法会通过重要性采样或 Critic 利用旧数据。
...
| 代表算法 | DQN（第 5 章）             | REINFORCE（本章） → PPO（第 8 章） |

两条路线各有优劣。好消息是，它们并非水火不容——第 7 章的 Actor-Critic 会把两条路线合并：用策略网络做决策，用价值网络降低方差。不过在此之前，我们需要先把 Policy-Based 这条路的数学基础打扎实。

## 策略目标函数

在讲怎么优化之前，先要回答一个更基本的问题：什么是"好的策略"？

在第 2 章的 MDP 框架中，我们定义了策略与回报；第 3 章进一步引入[状态价值函数](../chapter03_mdp/value-bellman) $V^\pi(s)$，衡量"从状态 $s$ 出发，遵循策略 $\pi$，期望能拿多少分"。现在我们把视角从"单个状态"拉到"整个策略"——不问"从某个状态出发好不好"，而是问"这个策略整体上有多好"。这正是第 2 章引入的策略目标函数 $J(\theta)$ 的含义：在所有可能的起点上，策略 $\pi_\theta$ 期望能累积的[折扣总奖励](../chapter03_mdp/mdp)。
...
这个区别带来了两个关键后果：Q-Learning 是 off-policy 的（可以用旧数据反复训练），REINFORCE 是 on-policy 的（必须用当前策略的新数据）；经典表格 Q-Learning 和 DQN 通过枚举动作取最大值，而 REINFORCE 可以用连续概率分布直接参数化连续动作。
