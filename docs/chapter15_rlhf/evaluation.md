...
经典 RLHF 主线到这里就完整闭环了：base model 不是 assistant，SFT 给它行为起点，RM 给它偏好方向，PPO 让它按奖励练习，评估和数据飞轮负责防止它学歪。如果想继续做一个受控实验，可以进入[扩展实战](./extended-practice)；下一章进入第 14 章，从这条经典 RLHF 流水线出发，讨论现代工业后训练如何调整奖励、优化器和分布式系统——[现代 LLM RL 工业实践](../chapter16_llm_rl_industrial/intro)。
