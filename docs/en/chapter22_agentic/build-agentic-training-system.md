# 20.10 Hands-on: Building an Agentic RL Training System from Scratch

In Sections 20.1 and 20.2, we discussed the decision framework and environment interaction design for Agentic RL. In Sections 20.3 through 20.5, we analyzed the architectures of frameworks such as OpenRLHF, veRL, and Relax. This section starts from those discussions and turns the concepts into a runnable implementation.
...
This isolated environment must satisfy three conditions: accept the agent's action (code), execute it safely with resource limits, and return the execution result and termination status. This is the responsibility of the **Environment** component, and it is the minimal implementation of the sandbox problem discussed in Section 20.2.
...
- `step()` accepts a structured action (`action_type` + `action_args`), not raw text. This corresponds to the action space $A = A_{\text{text}} \cup A_{\text{action}}$ from Section 20.2.
- `_exec_code()` uses subprocess isolation with a timeout to prevent infinite loops — the lightest sandbox approach discussed in Section 20.2.
...
class GRPOAgentTrainer:
    ...
    def _serialize_trajectory(self, traj: dict) -> str:
        ...
        See the loss mask discussion in Section 20.2.
        """
        parts = []
...
- `_serialize_trajectory()` flattens multi-turn trajectories into text. This is simplified — production frameworks use loss masks to distinguish model-generated tokens from environment-returned tokens (see the loss mask discussion in Section 20.2).
...
This section implemented a minimal yet complete Agentic RL training system. Looking back at the entire process, its core structure can be summarized as: **nesting the Agent Loop (Section 20.1) and environment interaction (Section 20.2) inside a rollout → reward → train RL cycle**. All the complexity of production frameworks like Relax and veRL arises from scaling this skeleton to multi-node multi-GPU, high-throughput, long-running production environments.
