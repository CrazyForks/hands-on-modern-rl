...
| 第 5 章 PPO          | $V(s)$、优势函数、TD Error、策略目标                                 | 用 Critic 估计优势，并约束策略更新幅度                  |
| 第 6 章以后大模型 RL | 策略、奖励、回报、目标函数                                           | 把 token 生成看作序列决策，把偏好或验证信号转成优化目标 |
...
下一章将从 $Q(s,a)$ 出发，进入第一个完整算法族：[第 4 章：深度 Q 网络](../chapter07_dqn/from-q-to-dqn)。
