...
Unlike preference alignment, tool-use reward is often not subjective. It can be designed from objective signals. In fact, this is a direct application of **RLVR (Reinforcement Learning from Verifiable Rewards)** from Chapter 16 to agentic settings.
...
<details>
<summary>Thinking question: What are the similarities and differences between the two-stage paradigm "SFT teaches format + RL teaches strategy" and DPO from Chapter 15?</summary>
...
In standard LLM RL training, such as PPO from Chapter 8 or GRPO from Chapter 16, the training loop is purely GPU-based: the model generates answers on the GPU, the reward model scores them on the GPU, and gradients are computed on the GPU. The slowest part is usually GPU computation.
...
| GRPO within-group comparison | Comparing multiple answers (Chapter 16)            | Comparing multiple trajectories (same idea applies)                   |
