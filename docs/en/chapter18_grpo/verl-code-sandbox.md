...
> **Objective of This Section**: Integrate a code execution validator into veRL, train a code model using test pass rate, and complete data preparation, sandbox checks, PPO training, and pre- and post-training evaluations.

> **Learning Path**: [15.3 RLVR Reward](./rlvr) → [13.7 veRL Training for GSM8K](../chapter15_rlhf/verl-ppo-gsm8k) → **15.8 veRL Training for Code Generation**

> **Code for This Section**: [Data Preparation](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter18_grpo/verl_code_rlvr/prepare_data.py) · [Code Reward](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter18_grpo/verl_code_rlvr/code_reward.py) · [Single-GPU Training Script](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter18_grpo/verl_code_rlvr/run_qwen_coder_ppo_single_gpu.sh)
...
The original tutorial from VolcEngine used VKE cluster + SandboxFusion cloud sandbox for large-scale distributed training. This section adapts these solutions to a **local GPU environment**: using subprocess isolation instead of cloud sandbox, and using single- or multi-GPU scripts instead of cluster deployment, while retaining the same algorithm logic and parameter configuration. The complete industrial-level code agent experiment is placed in [19.8 Training DeepCoder Agent with rLLM](../chapter22_agentic/rllm-deepcoder-lab), where the focus is on AgentFlow and sandbox cookbook. This section focuses more on how to integrate a code verifier into the veRL training framework.
...
## Why Code Generation is Suitable for RLVR
...
Using `subprocess` to start a real child process for execution is safer than `exec`: full process isolation, so the model's dead loops, file operations, and network requests do not affect the training process:
...
> **Note**: The data in the table is from the official experiment conducted by VolcEngine in a multi-GPU environment. The model in this section is smaller and trained with fewer steps on a single GPU, so the specific numbers may differ, but the training dynamics and trends are consistent.
...
## Scaling from Single GPU to Multi-GPU
...
## Relationship with the DeepCoder Experiment
...
- Local subprocesses must be set with timeouts and resource limits; the training set and evaluation set must be isolated.
