# 20.10 动手：从零实现一个 Agentic RL 训练系统

在 20.1 节和 20.2 节中，我们讨论了 Agentic RL 的决策框架和环境交互的设计思想；在 20.3 节至 20.5 节中，我们分析了 OpenRLHF、veRL、Relax 等框架的架构。本节将从这些讨论出发，用可运行的代码把这些概念变成具体实现。
...
这种隔离环境需要满足三个条件：接收 Agent 的动作（代码）、安全执行并限制资源、返回执行结果和终止状态。这就是 **Environment** 组件的职责，也是 20.2 节讨论的沙箱问题的最小实现。
...
- `step()` 接受结构化的 action（`action_type` + `action_args`），不是纯文本。这对应 20.2 节的动作空间 $A = A_{\text{text}} \cup A_{\text{action}}$
...
到这一步，我们已经能收集完整的交互轨迹了。但光有轨迹还不够——我们需要把它们变成梯度，更新模型参数。回顾第 16 章，GRPO 的核心思想是对同一个 prompt 采样多条轨迹，在组内做比较来计算 advantage。
...
本节实现了一个最小但完整的 Agentic RL 训练系统。回顾整个过程，其核心结构可以概括为：**将 Agent Loop（20.1）和环境交互（20.2）嵌套进一个 rollout → reward → train 的 RL 循环中**。Relax、veRL 等生产框架的所有复杂性，都来自于将这个骨架扩展到多机多卡、高吞吐、长时间运行的生产环境。
