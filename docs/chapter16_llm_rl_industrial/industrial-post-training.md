...
[18.1](./intro) 已经把一次 RL 训练拆成生成、奖励、更新和参数同步。工业后训练还要解决一个更大的问题：训练数据从哪里来，模型学完以后怎样判断能力是否真的提高，以及失败样本怎样回到下一轮训练。

先看一个代码模型的例子。团队从真实仓库中收集问题，为每个问题准备可以运行的环境和测试。模型先通过 SFT 学会读取仓库、调用工具和提交修改，再通过 RL 反复尝试；测试程序给每次尝试打分。训练完成后，团队在没有参加训练的新仓库上评测模型，把失败任务归类，补充新的数据和验证规则，然后开始下一轮训练。
...
不同公司会使用不同算法名称，完整流程都可以放回这六个阶段理解。下面的公司资料保留为案例库，第一次阅读时先看“方法主线”即可。

## 截至 2026 年 8 月，公开方案出现了哪些变化

早期公开方案常把一批数学题交给模型作答，再按最终答案计算奖励。最新的公开资料把训练对象扩展到了代码仓库、网页、桌面应用和长期运行的智能体。变化主要集中在四个地方。

1. **一个任务现在带着自己的环境。** 代码任务需要仓库、依赖和测试；网页任务需要浏览器状态与工具；桌面任务还要保存文件和应用状态。只有任务文本，已经无法重放一次训练。
2. **不同能力可以先分别训练，再合回一个模型。** 面壁智能 MiniCPM5、Moonshot Kimi K3 和 NVIDIA Nemotron-Cascade 2 都公开了“领域 RL 专家 + On-Policy Distillation”的路线。数学、代码或写作专家先各自学习，发布前再把能力汇总到同一个模型。
3. **环境差异开始进入训练数据。** MiniMax 使用不同 agent scaffold 采样同一类任务；Kimi K3 会改变工具、上下文管理和提示组成；Qwen-AgentWorld 同时使用容器、MCP、移动端和网页环境。这样做是为了减少模型对某一种执行模板的依赖。
4. **训练和部署使用同一套约束。** 长度预算、思考强度、量化方式、工具协议和上下文管理逐渐在训练阶段就固定下来。模型学到的不只是“答对”，还包括在真实预算和执行条件下完成任务。

这些变化背后都指向同一个基础设施：团队必须记录任务、环境、轨迹、奖励和模型版本，才能重放失败、比较实验并继续训练。[18.5 大规模 RL 数据工程](./data-engineering) 将单独梳理这条生产线。

:::: details 最新公开案例：MiniCPM5、Kimi K3、Qwen-AgentWorld 与 GLM-5.2

### 面壁智能 MiniCPM5：RL 专家先分工，再用 OPD 汇总
...
[18.3 训练为什么会不稳定](./modern-industrial-practice) 将沿着这条闭环检查训练信号：哪些曲线说明模型正在学习，哪些变化意味着奖励、优化或系统已经出现问题。

:::: details 参考资料：公司报告、论文与开源项目
