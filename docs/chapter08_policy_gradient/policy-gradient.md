...
在第 3 章的 MDP 框架中，我们定义过[状态价值函数](../chapter03_mdp/value-bellman) $V^\pi(s)$——衡量"从状态 $s$ 出发，遵循策略 $\pi$，期望能拿多少分"。现在我们把视角从"单个状态"拉到"整个策略"——不问"从某个状态出发好不好"，而是问"这个策略整体上有多好"。这正是第 3 章引入的[策略目标函数](../chapter08_policy_gradient/policy-gradient) $J(\theta)$ 的含义。答案很自然：在所有可能的起点上，策略 $\pi_\theta$ 期望能累积的[折扣总奖励](../chapter03_mdp/mdp)。
