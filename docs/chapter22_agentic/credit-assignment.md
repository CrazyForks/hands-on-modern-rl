...
[20.2 多轮 RL 形式化](./formulation) 把轨迹概率写成 $\rho_\theta(x | s_0, c) = \mathbb{I}(s_0, x) \prod_{t \in a(x)} p_\theta(x_t | \cdot)$，并推出 action mask——只有 action token 参与 policy gradient。但这只回答了"该在哪些 token 上算梯度"，没回答"每个 action token 该乘以多大的 advantage"。
...
下一节我们来拆解 Agentic RL 的工程核心——[20.4 工具调用 RL](./tool-use-and-trajectory)，看看训练数据从哪里来、工具策略怎么学、系统怎么跑起来。
