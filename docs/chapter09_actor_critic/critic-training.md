...
沿用第 4 章的三格走廊环境，固定策略 $\pi$：在 $S$ 和 $M$ 均以 0.8 概率右走、0.2 概率左走。环境转移和奖励如下：
...
实际中，Actor-Critic 几乎都用 TD 方法来训练 Critic。在更高级的实现中（如[第 8 章的 GAE](../chapter10_ppo/gae-reward-model)），MC 和 TD 会被组合使用——通过参数 $\lambda$ 在两者之间插值，获得偏差和方差的最佳平衡。
