# 补充阅读：Post-Training 流水线与 DPO 推导
...
### 14.1.2 阶段二：监督微调（Supervised Fine-Tuning, SFT）
...
### 14.1.3 阶段三：强化学习与对齐（RL / Alignment）
...
- **数据示例（偏好三元组）**——典型的偏好对比数据格式（与 [1-generate_data.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/1-generate_data.py) 生成的格式一致）：
  ```json
  {
    "prompt": "我今天心情很差，不想上班。",
...
回到我们在上一节中运行的 [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py)，代码里加载的 `preference_data.json` 正是这种格式。数据加载部分将 JSON 文件解析为 `prompt`、`chosen`、`rejected` 三个字段，分别对应符号表中的 $x$、$y_w$、$y_l$：
...
2. **用强化学习算法更新策略模型（Policy Model）**：让大模型不断生成回答，奖励模型给它打分，然后根据分数高低来调整大模型的参数。最常用的算法是第 8 章讲过的 PPO（Schulman et al., 2017）。
...
要理解为什么能跳过，需要回顾 PPO 训练时的一个隐含约束：策略不能偏离原始模型太远，这一约束用 KL 散度来衡量。如果完全不加约束，模型可能为了拿到高奖励而输出乱码——奖励分数很高，但人类完全看不懂。这与第 8.2 节讨论的 PPO 裁剪机制本质上出于同样的考量：限制每次更新的幅度，保证训练稳定性。
...
回到 [3-train_dpo.py](https://github.com/walkinglabs/hands-on-modern-rl/blob/main/code/chapter17_dpo/3-train_dpo.py) 的代码，`DPOTrainer` 在初始化时只接收一个 `model` 参数：
