...
例如：训练 GPU 在第 100 步更新了参数，同步要 20ms，这期间推理 GPU 还在用第 99 步的模型生成数据。这些数据是"旧策略"生成的，用它来更新"新策略"，严格来说有偏差——这就是 off-policy 问题（你在本书 [第 6 章](/chapter08_policy_gradient/intro) 学过）。
