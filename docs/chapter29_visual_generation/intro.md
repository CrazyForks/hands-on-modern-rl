...
| 第 28 章 Reward Hacking | 生成模型可能讨好 reward model，却牺牲真实用户意图           |
| 第 16 章 RLVR           | 细粒度属性、数量、关系可以变成局部可验证信号                |
| 第 20 章 Agentic RL     | 长 horizon 信用分配、多组件 reward 和 KL 约束都再次出现     |
| 第 24.1-24.4 节 VLM RL | VLM 可以反过来做生成模型的 judge、captioner 和 reward model |
...
到这里，我们覆盖了视觉理解与视觉生成两个方向的 RL 训练。下一章进入[第 28 章奖励黑客与 RL 评估](../chapter30_alignment_failures/classical-failures)，讨论能力扩展之后必须面对的对齐失败与评估问题。
