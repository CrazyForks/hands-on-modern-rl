...
# 20.5 Search-Augmented RL 与 Web/Code Agent
...
RLER 的实现方式是：维护一个评分标准版本库，每隔 $N$ 步训练就根据模型的当前表现调整评分标准的严格程度。这和[20.4 节](./tool-use-and-trajectory)讨论的课程学习有相似的思想——但 RLER 是"评分标准在演化"，而不是"任务难度在增加"。
