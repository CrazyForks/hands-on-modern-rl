...
你会发现训练脚本会持续打印各种数值。
下面这段摘自一次真实训练日志
（2026-08-13，修复 GAE 回合边界后的完整运行）：

```
------------------------------------------------------------
  迭代  1/40 | 回合数:  85 | 平均奖励:   23.9 | KL: 0.0078 | clip%: 11.0%
  迭代  7/40 | 回合数:  10 | 平均奖励:  202.5 | KL: 0.0053 | clip%: 4.6%
  迭代 12/40 | 回合数:   4 | 平均奖励:  491.5 | KL: 0.0069 | clip%: 8.5%
  迭代 13/40 | 回合数:   4 | 平均奖励:  500.0 | KL: 0.0014 | clip%: 1.2%
  迭代 40/40 | 回合数:   4 | 平均奖励:  500.0 | KL: 0.0000 | clip%: 0.0%
------------------------------------------------------------
...
训练完成后得到的奖励曲线如下图所示，
展示了修复后的纯 PyTorch PPO
在 CartPole-v1 上的一次完整训练：

![回合平均奖励曲线](./images/training_curves.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>图 1-3：修复后的 PyTorch PPO 实测曲线。回合平均奖励从 23.9 分上升至满分 500，虚线为 CartPole-v1 的 475 分解题线。</em>
</div>
...
2. **快速上升阶段（5K ~ 25K Total Timesteps）**：
   奖励从不到 100 快速上升，
   表明策略开始掌握平衡控制。
   第 12 轮平均奖励达到 491.5，首次越过 475 分解题线。
3. **收敛阶段（25K Total Timesteps 之后）**：
   奖励进入 `400 ~ 500` 区间，并在后期越过 475 分解题线，
   最终稳定在满分 `500`。
   两个实现最终均达到了 `500.0 +/- 0.0` 的评估结果。

总结而言：**曲线的移动平均整体上升，并在高回报区间趋于稳定，表明训练取得了较好结果。**
若曲线始终持平或出现突然暴跌，
则说明训练过程存在异常。
